...
```sh
deno run --allow-read 06p1.ts test
deno run --allow-read 06p1.ts
deno run --allow-read 06p2.ts test
deno run --allow-read 06p2.ts
```

Could be more efficient, but as it still takes only ~100 ms on my machine, I guess it's fine.
