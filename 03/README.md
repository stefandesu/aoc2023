...
```sh
deno run --allow-read 03p1.ts test
deno run --allow-read 03p1.ts
deno run --allow-read 03p2.ts test
deno run --allow-read 03p2.ts
```

Not a very clean solution... in particular, part 2 is simply a copy of part 1 with stuff put on top, so it's even less clean. But it works. 🤷
