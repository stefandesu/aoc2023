...
```sh
deno run --allow-read 04p1.ts test
deno run --allow-read 04p1.ts
deno run --allow-read 04p2.ts test
deno run --allow-read 04p2.ts
```
...
As far as the Elf has been able to figure out, you have to figure out which of the _numbers you have_ appear in the list of _winning numbers_. The first match makes the card worth _one point_ and each match after the first _doubles_the point value of that card.
